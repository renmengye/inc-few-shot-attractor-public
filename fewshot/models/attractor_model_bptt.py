"""A transfer model using BPTT to learn.
...
from fewshot.utils.checkpoint import update_add_checkpoint
...
class AttractorModelBPTT(AttractorModel):
  ...
  def __init__(self,
    ...
    with tf.name_scope('TaskB'):
      self.build_task_b(x_b, y_b, x_b_v, y_b_v, y_sel)
      if is_training:
  ...
  def build_task_b(self, x_b, y_b, x_b_v, y_b_v, y_sel):
    """Build task B.
    ...
    for v in fast_weights:
      # Zero initialization
      # init_ops.append(
      #     tf.assign(v, tf.zeros([int(ss) for ss in v.shape], dtype=v.dtype)))
    ...
    self.fast_weights0 = fast_weights0  # Copy the list items.
    dummy_lr = transfer_config.dummy_lr
    # dummy_lr_bptt = transfer_config.dummy_lr
    dummy_lr_bptt = transfer_config.ft_optimizer_config.lr_list[0]
    meta_cost_bptt = 0.0
    cost_b_list = []
    acc_tr_list = []
    acc_v_list = []
    fast_weights_list = []
    acc_v = tf.constant(0.0)
    for step in range(transfer_config.bptt_steps):
      ...
      if transfer_config.old_and_new:
        ...
        pred_b_ = tf.argmax(logits_b[:, num_classes_a:], axis=-1)
      else:
        y_b_tr = y_b
        pred_b_ = tf.argmax(logits_b, axis=-1)

      correct_b_ = tf.cast(tf.equal(pred_b_, y_b_tr), logits_b.dtype)
      acc_tr = tf.reduce_mean(correct_b_)
      ...
      with tf.control_dependencies([cost_b, acc_tr, acc_v]):
      ...
      if step == 0:
        # RBP dummy gradient descent.
        fast_weights_rbp = [
            vv - dummy_lr * gg if gg is not None else vv
            for gg, vv in zip(fast_grads, fast_weights)
        ]
        fast_weights1 = fast_weights_rbp
        self.fast_weights1 = fast_weights1
        self._cost_b = cost_b
        logits_b_v = self.compute_logits_b_all(h_b_v, fast_weights_rbp,
                                               w_class_a_mask, b_class_a)
        xent_b_v = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=logits_b_v, labels=y_b_v))
        self._cost_b_v = xent_b_v
        self.meta_cost_rbp = xent_b_v

      # BPTT dummy gradient descent.
      fast_weights = [
          vv - dummy_lr_bptt * gg if gg is not None else vv
          for gg, vv in zip(fast_grads, fast_weights)
      ]
      fast_weights_list.append(fast_weights)
      logits_b_v_bptt = self.compute_logits_b_all(h_b_v, fast_weights,
                                                  w_class_a_mask, b_class_a)
      pred_b_v_ = tf.argmax(logits_b_v_bptt, axis=-1)
      correct_b_v_ = tf.cast(tf.equal(pred_b_v_, y_b_v), logits_b.dtype)
      acc_v = tf.reduce_mean(correct_b_v_)
      acc_v_list.append(acc_v)
      xent_b_v_bptt = tf.reduce_mean(
          tf.nn.sparse_softmax_cross_entropy_with_logits(
              logits=logits_b_v_bptt, labels=y_b_v))
      meta_cost = xent_b_v_bptt

      if transfer_config.bptt_all:
        num_steps = float(transfer_config.bptt_steps)
        meta_cost_bptt += meta_cost / num_steps * transfer_config.bptt_ratio
      elif step == transfer_config.bptt_steps - 1:
        meta_cost_bptt = meta_cost * transfer_config.bptt_ratio

    if not transfer_config.steady_rbp:
      self._cost_b_v = xent_b_v_bptt  # Use transient state to eval.
      self._cost_b = cost_b

    self.cost_b_list = tf.stack(cost_b_list, axis=0)
    self.acc_tr_list = tf.stack(acc_tr_list, axis=0)
    self.acc_v_list = tf.stack(acc_v_list, axis=0)

    # [T, wb[i].shape]
    self.fast_weights_list = [
        tf.stack([fw[i]
                  for fw in fast_weights_list], axis=0)
        for i in range(len(fast_weights))
    ]
    print('all fast weights', self.fast_weights_list)
    # assert False
    self.meta_cost_bptt = meta_cost
    ...
    if transfer_config.steady_rbp:
      ...
      self._prediction_b = logits_b_v_bptt
  ...
  def compute_logits_b(self, h, weights):
    ...
    elif transfer_config.fast_model == 'mlp':
      logits_b_v = self.mlp(h, *weights)
    elif transfer_config.fast_model == 'resmlp':
      logits_b_v = self.resmlp(h, *weights)
    elif transfer_config.fast_model == 'reskmlp':
      logits_b_v = self.reskmlp(h, *weights)
    elif transfer_config.fast_model == 'kmlp':
      logits_b_v = self.kmlp(h, *weights)
    return logits_b_v
  ...
  def build_task_b_grad(self, x_b_v, y_b_v, y_sel):
    ...
    grads_b_bptt = tf.gradients(self.meta_cost_bptt, x, gate_gradients=1)
    grads_b_rbp = rbp(
        x,
        fast_weights1,
        fast_weights0,
    ...
    [
        ...
        for x, grad in zip(meta_weights, grads_b_rbp)
    ]
    self._meta_weights = meta_weights
    self._grads_meta = grads_b_rbp
    update_grads_b_bptt = write_checkpoint(grads_b_ckpt, grads_b_bptt)
    if transfer_config.transient_bptt:
      update_grads_b = update_add_checkpoint(grads_b_ckpt, grads_b_rbp)
    else:
      update_grads_b = write_checkpoint(grads_b_ckpt, grads_b_rbp)
    self._update_grads_b = update_grads_b
    self._update_grads_b_bptt = update_grads_b_bptt
  ...
  def bptt_b(self,
    ...
    _, cost_b = sess.run([self._update_grads_b_bptt, self._cost_b],
                         feed_dict=fdict)
    return cost_b
  ...
  def monitor_b(self,
    ...
    Args:
      ...
      run_fast_weights: Whether also output the fast weights during training.
    """
    ...
    if run_fast_weights:
      out = [
          ...
          self.acc_tr_list,
          self.acc_v_list,
          self.attractor.attended_h,
          self.attractor.protos,
          self.fast_weights_list,
      ]
      results = sess.run(out, feed_dict=fdict)
      w_class_a = sess.run(self.w_class_a)
      cost_b_list = results[0]
      acc_tr_list = results[1]
      acc_v_list = results[2]
      attr_list = results[3]
      proto_list = results[4]
      fast_weights_list = results[5:]
      return (cost_b_list, acc_tr_list, acc_v_list, attr_list, proto_list,
              w_class_a, fast_weights_list)
    else:
      cost_b_list, acc_tr_list, acc_v_list = sess.run(
          [self.cost_b_list, self.acc_tr_list, self.acc_v_list],
          feed_dict=fdict)
      return cost_b_list, acc_tr_list, acc_v_list

  def solve_b(self, sess, x_b_np, y_b_np, fdict=None, reset=True):
    ...
    # Run 2nd order optimizers (through scipy).
  ...
  def eval_step_b(self, sess, task_b_data):
    ...
    fdict = self._prerun(sess, None, task_b_data)
    if self.config.transfer_config.steady_rbp:
      _ = self.solve_b(
          sess, task_b_data.x_train, task_b_data.y_train, fdict=fdict)
    batch_size = self.config.transfer_config.ft_optimizer_config.batch_size
    if task_b_data.x_test.shape[0] > 300:  # Cap at 300.
      batch_size = 300
    if batch_size == -1:
      prediction_b, y_b = sess.run(
          [self.prediction_b_all, self.labels_b_v_all], feed_dict=fdict)
    else:
      # Run evaluation in mini-batches (GPU memory bound).
      nbatches = int(np.ceil(task_b_data.x_test.shape[0] / batch_size))
      y_b = []
      prediction_b = []
      it = six.moves.xrange(nbatches)
      it = tqdm(it, ncols=0, desc='running eval b')
      for num in it:
        x_, y_ = self.minibatch(
            task_b_data.x_test, task_b_data.y_test, batch_size, num=num)
        fdict[self.inputs_b_v] = x_
        fdict[self.labels_b_v] = y_
        pred_b_, y_b_ = sess.run([self.prediction_b_all, self.labels_b_v_all],
                                 feed_dict=fdict)
        y_b.append(y_b_)
        prediction_b.append(pred_b_)
      y_b = np.concatenate(y_b, axis=0)
      prediction_b = np.concatenate(prediction_b, axis=0)
    return prediction_b, y_b

  def eval_curve_b(self, sess, task_b_data, run_fast_weights=False):
    """Evaluate one episode with curves."""
    fdict = self._prerun(sess, None, task_b_data)
    results = self.monitor_b(
        sess,
        task_b_data.x_train,
        task_b_data.y_train,
        task_b_data.x_test,
        task_b_data.y_test,
        run_fast_weights=run_fast_weights,
        fdict=fdict)
    return results
  ...
  def train_step(self, sess, task_a_data, task_b_data):
    """Train a single step."""
    fdict = self._prerun(sess, None, task_b_data)
    assert self.config.transfer_config.transient_bptt, 'hey'
    if self.config.transfer_config.transient_bptt:
      cost_b = self.bptt_b(
          sess,
          task_b_data.x_train,
          task_b_data.y_train,
